...
asio::awaitable<void> async_main(CoroHttpClient& client) {
    std::cout << "=== Test: Compression Support ===" << "\n";
    auto gzip_resp = co_await client.co_get("https://httpbin.org/gzip");
    ...
    auto redirect_resp = co_await client.co_get("https://httpbin.org/redirect/3");
    ...
    HttpRequest custom_req(HttpMethod::GET, "https://httpbin.org/headers");
    ...
    HttpRequest post_req(HttpMethod::POST, "https://httpbin.org/post");
    post_req.add_header("Content-Type", "application/json")
            .set_body(R"({"name":"test","value":123,"nested":{"key":"value"}})");
    auto post_resp = co_await client.co_execute(post_req);
    std::cout << "Status: " << post_resp.status_code() << "\n";
    std::cout << "Response length: " << post_resp.body().length() << "\n\n";

    std::cout << "=== Test: Response Headers ===" << "\n";
    auto headers_resp = co_await client.co_get("https://httpbin.org/response-headers?Custom-Header=CustomValue");
    ...
    try {
        auto delay_resp = co_await client.co_get("https://httpbin.org/delay/10");
    ...
    std::cout << "PUT: " << (co_await client.co_put("https://httpbin.org/put", R"({"updated":true})")).status_code() << "\n";
    std::cout << "DELETE: " << (co_await client.co_delete("https://httpbin.org/delete")).status_code() << "\n";
    std::cout << "PATCH: " << (co_await client.co_patch("https://httpbin.org/patch", R"({"patched":true})")).status_code() << "\n";
    std::cout << "HEAD: " << (co_await client.co_head("https://httpbin.org/get")).status_code() << "\n";
}
...
int main() {
    try {
        ...
        asio::co_spawn(io_ctx, async_main(client), asio::detached);
        io_ctx.run();
